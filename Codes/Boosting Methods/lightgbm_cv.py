...
if is_pca:
    ...
    if os.path.isfile(data_path + 'lgb_training_data.pkl'):
        print('* Found previously preprocessed lgb dataset. Loading...')
        with open(data_path + 'lgb_training_data.pkl','rb') as f:
            lgtrain = pickle.load(f)
        with open(data_path + 'lgb_testing_data.pkl','rb') as f:
            x_test = pickle.load(f)
    else:
        ## Read in data
        print('* Reading in training and testing data from csv, this may take a while...')
        ...
        lgtrain = lgb.Dataset(x_train,label = y_train)
        print('* Data converted into lgb dataset')
        save_object(lgtrain, data_path + 'lgb_training_data.pkl')
        save_object(x_test, data_path + 'lgb_testing_data.pkl')
# In[20]:
...
def tune_ind_params(params,data,nfold,return_best=is_return_best,**kwargs):
...
def lgb_cv_tuning(grid,data,nfold,return_best=is_return_best,parallel=True,**kwargs):
    ...
    if parallel:
        ...
        print('* Begin CV')
        cv_results = Parallel(n_jobs=50)(delayed(tune_ind_params)(all_params[i],data,nfold) for i in range(len(all_params)))
    else:
...
if is_return_best:
    ...
    print(best_cv)
    save_object(best_cv, data_path + 'lgb_best_cv_pca_005_100_30000.pkl')
else:
    cv = lgb_cv_tuning(params_grid,lgtrain,nfold=5)
    save_object(cv, data_path + 'lgb_cv_history_pca_005_100_30000.pkl')
...
# save_object(model, data_path + 'lgb_model_nopca_005_100_30000.pkl')
#
# print('* Making final prediction on testing dataset...')
# pred = model.predict(x_test)
# save_object(pred, data_path + 'lgb_pred.pkl')
