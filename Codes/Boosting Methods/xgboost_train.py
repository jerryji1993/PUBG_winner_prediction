...
def feature_engineering(df,is_train=True,debug=True,**kwargs):
    if is_train:
...
def tune_ind_params(params,data,nfold,niter,earlystopping,**kwargs):
...
def xgb_cv_tuning(grid,data,nfold,niter,earlystopping,return_best=True,parallel=False,**kwargs):
    ...
    if parallel:
        ...
        print('* Begin CV')
        cv_results = Parallel(n_jobs=num_cores)(delayed(tune_ind_params)(all_params[i],data,nfold,niter,earlystopping) for i in range(len(all_params)))
    else:
...
save_object(best_params, data_path + 'xgb_best_params_005_15_30000.pkl')
